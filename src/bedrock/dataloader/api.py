...
from bedrock.core.io import write_source_file, write_source_config, write_source_files_web
from bedrock.core.models import Source, SourceFolder
...
api = Api(app, version="0.1", title="DataLoader API",
          description="Analytics-Framework API supporting creation and use of matrices")
...
api.model('Filter', {
    ...
    'possible_names': fields.List(fields.String,
                                  description='List of names that could indicate appropriateness for this filter',
                                  required=True),
...
@api.model(fields={
    ...
    'suggestions': fields.List(fields.String, description='List of other possible filters for this field',
                               required=True),
...
class Sources(Resource):
            ...
            try:
                ...
                for matrix in matrices:
                    if matrix['id'] == matrix_id or matrix['name'] == matrix_id:
                        return send_from_directory(matrix['rootdir'], output_file, as_attachment=True,
    ...
    class NewSource(Resource):
        ...
        def put(self, name, ingest_id, group_name=""):
            ...
            # Load OSF-based directory of files for analysis
            if ingest_id == "opals.cycle2_1_load.Load.Load":
                # Check the post for credentials
                ...
                if existing_source is not None and not overwrite:
                    logging.warn("SourceFolder Already Exists: {}".format(existing_source['src_id']))
                    existing_source['error'] = 1
                    existing_source['msg'] = "SourceFolder Already Exists"
                    return existing_source, 201
                ...
                try:
                        ...
                        for f_name, url in all_names:
                            try:
                                ...
                                if not ext in ALLOWED_EXTENSIONS:
                                    print("WARN: File submitted %s is not of a supported filetype".format(f_name))
                    ...
                    source = SourceFolder(name, rootpath, src_id, src_type, t, ingest_id, group_name)
                    source_insert_response = col.insert_one(source.dict())
                    if source_insert_response is False:
                        logging.error("Source Folder Insert Failed")
                        tb = traceback.format_exc()
                        return tb, 406
    ...
    class Source(Resource):
                ...
                try:
                    ...
                    for mat in matrices:
                        ...
                        try:
                            ...
                            logging.error('could not remove matrix results %s while deleting source %s exception:%s',
                                          mat_id, src_id, ex)
        ...
        class Custom_2(Resource):
            def get(self, src_id, param1, param2, param3=None):
                ...
                return utils.custom(ingest_id=src['ingest_id'], filepath=filepath, param1=param1, param2=param2,
                                    param3=param3,
        ...
        class Custom_1(Resource):
            def get(self, src_id, param1):
                ...
                try:
                    ...
                    return 'No resource at that URL.', 404
                filepath = src['rootdir']
                return utils.custom(ingest_id=src['ingest_id'], filepath=filepath, param1=param1, request=request)
