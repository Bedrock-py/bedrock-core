...
br_reload () {
    ...
    do
        ...
        echo "python configure.py --mode reload --api $INTERFACE --filename $FILE" > $LOGFILE
    ...
    if [ "$OPAL" = "application" ]; then
        ...
        do
            ...
            if ! [ -z "$f" ]; then
                ...
                rm "$TARGET$FILE"
                echo "python configure.py --mode remove --api $INTERFACE --filename $FILE" > $LOGFILE
...
br_install () {
    ...
    do
        if ! [ -z "$f" ]; then
            ...
            FILE=""
            myln "$f" "$TARGET" "$OPAL"
            echo "python configure.py --mode add --api $INTERFACE --filename $FILE" > "$LOGFILE"
