...
if [ -z "$BEDROCK_DIR" ]; then
    # BEDROCK_DIR=/home/vagrant/bedro
    BEDROCK_DIR=~/bedrock/
    # echo "test" 
    # echo $(BEDROCK_DIR)
fi 
...
if [ $1 = "-h" ]; then
    ...
    exit 0
        ...
        rm analytics/opals/*.py* 2>/dev/null
        rm analytics/*.pyc 2>/dev/null
        touch analytics/opals/__init__.py


        rm dataloader/opals/*.py* 2>/dev/null
        rm dataloader/*.pyc 2>/dev/null
        touch dataloader/opals/__init__.py

        rm visualization/opals/*.py* 2>/dev/null
        rm visualization/*.pyc 2>/dev/null
        touch visualization/opals/__init__.py

        rm *.pyc 2>/dev/null

        rm -r analytics/data/*
        rm -r dataloader/data/*
    ...
    if [ -z $2 ]; then
        ...
        rm analytics/opals/*.pyc 2>/dev/null
        rm analytics/*.pyc 2>/dev/null
        touch analytics/opals/__init__.py

        rm dataloader/opals/*.pyc 2>/dev/null
        rm dataloader/*.pyc 2>/dev/null
        touch dataloader/opals/__init__.py

        rm visualization/opals/*.pyc 2>/dev/null
        rm visualization/*.pyc 2>/dev/null
        touch visualization/opals/__init__.py
...
if ! [ $2 = "application" ]; then
    ...
    if [ ! -d "$BEDROCK_DIR/$2" ]; then
        ...
        OPALS=$(curl --silent $OPALSERVER/opals/) #sets OPALS to the contents of master_confug.json
...
if [ $1 = "install" ]; then
    ...
    if [ $2 = "application" ]; then
        ...
        sed -e /$2/d -i installed_opals.txt
